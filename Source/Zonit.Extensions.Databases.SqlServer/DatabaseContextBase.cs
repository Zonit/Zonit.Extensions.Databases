...
/// </summary>
/// <remarks>
/// Initializes a new instance of the <see cref="DatabaseContextBase"/> class.
...
public abstract class DatabaseContextBase(DbContextOptions options) : DbContext(options)
{
    /// <summary>
    /// Gets whether extension properties should be automatically ignored (no [NotMapped] needed).
    /// Default is true. Override and return false to disable automatic ignoring.
    /// </summary>
    protected virtual bool AutoIgnoreExtensionProperties => true;
    ...
    {
        // Automatically ignore extension properties (User, Organization, etc.)
        // No [NotMapped] attributes needed on extension properties
        if (AutoIgnoreExtensionProperties)
...
[RequiresUnreferencedCode("EF Core isn't fully compatible with trimming.")]
[RequiresDynamicCode("EF Core isn't fully compatible with NativeAOT.")]
public abstract class DatabaseContextBase<TContext> : DbContext where TContext : DbContext
{
    /// <summary>
    /// Initializes a new instance of the <see cref="DatabaseContextBase{TContext}"/> class.
    /// </summary>
    /// <param name="options">The options for this context.</param>
    protected DatabaseContextBase(DbContextOptions<TContext> options) : base(options)
    {
    }
